/*
 * PhyloGraph.java Copyright (C) 2024 Daniel H. Huson
 *
...
public class PhyloGraph extends Graph {
	...
	public double getConfidence(Edge e) {
		return edgeConfidences == null ? DEFAULT_CONFIDENCE : edgeConfidences.getOrDefault(e, DEFAULT_CONFIDENCE);
	}
	...
	public double getProbability(Edge e) {
		return edgeProbabilities == null ? DEFAULT_PROBABILITY : edgeProbabilities.getOrDefault(e, DEFAULT_PROBABILITY);
	}
	...
	public void addTaxon(Node v, int taxId) {
		if (taxId < 0)
			throw new IllegalArgumentException("taxId must be non-negative");
	...
	public void add(PhyloGraph graph) {
		try (NodeArray<Node> old2new = new NodeArray<>(graph)) {
			...
			for (var e : edges()) {
				...
				if (graph.hasEdgeWeights() && graph.edgeWeights.get(e) != null)
					setWeight(f, graph.getWeight(e));
				if (graph.hasEdgeConfidences() && graph.edgeConfidences.get(e) != null)
					setConfidence(f, graph.getConfidence(e));
				if (graph.hasEdgeProbabilities() && graph.getEdgeProbabilities().get(e) != null)
					setProbability(f, graph.getProbability(e));
			}
