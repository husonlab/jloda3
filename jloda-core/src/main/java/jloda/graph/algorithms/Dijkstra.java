/*
 * Dijkstra.java Copyright (C) 2026 Daniel H. Huson
 *
...
public class Dijkstra {
     ...
     */
    public static List<Node> compute(final Graph graph, Node source, Node sink, ToDoubleFunction<Edge> weights, boolean undirected) {
     ...
     * @param weights    function mapping an edge to a weight (non-negative)
     * @param undirected if true, treat edges as undirected (use both outEdges and inEdges)
     * @return distance from source to sink
     */
    public static double computeDistance(final Graph graph, Node source, Node sink, ToDoubleFunction<Edge> weights, boolean undirected) {
        try (NodeArray<Node> predecessor = graph.newNodeArray();
             var dist = graph.newNodeDoubleArray()) {
            ...
            if (Double.isInfinite(dist.get(sink)))
                throw new RuntimeException("No path from source to sink");
            return dist.get(sink);
        }
    }
