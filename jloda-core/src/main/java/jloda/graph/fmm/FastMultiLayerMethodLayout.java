/*
 * FastMultiLayerMethodLayout.java Copyright (C) 2024 Daniel H. Huson
 *
...
public class FastMultiLayerMethodLayout {
	 ...
	 * @param edgeWeights0       optional edge weights (may be null for unit lengths)
	 * @param initialPosFunction Optional function for providing initial positions
	 * @param result             the resulting node coordinates
	 */
	public static Rectangle apply(FastMultiLayerMethodOptions options,
								  Graph graph,
		...
		ToDoubleFunction<Edge> defaultWeights = e -> 1.0;
		...
		if (graph.getNumberOfNodes() > 2) {
			...
			graph.nodeStream().forEach(v -> {
				...
				if (initialPosFunction != null) {
					var point = initialPosFunction.apply(v);
					attributes.setPosition(point.getX(), point.getY());
				}
				nodeAttributes.put(v, attributes);
			});
			graph.edgeStream().forEach(e -> edgeAttributes.put(e,
					new EdgeAttributes(edgeWeights.applyAsDouble(e))));

			// Set ideal edge lengths (with floor/cap as requested)
			...
			double base = options.getUnitEdgeLength();
			double weight;
			if (graph.getNumberOfEdges() == 1)
				weight = edgeWeights.applyAsDouble(graph.getFirstEdge());
			else if (graph.getNumberOfEdges() == 2)
				weight = 0.5 * (edgeWeights.applyAsDouble(graph.getFirstEdge())
								+ edgeWeights.applyAsDouble(graph.getLastEdge()));
			else weight = 1.0;
	...
	/* ====================== Edge length initialization with floor & cap ====================== */

	private static void initializeIdealEdgeLengths(Graph graph,
		...
		if (options.getEdgeLengthMeasurement() == FastMultiLayerMethodOptions.EdgeLengthMeasurement.MidPoint) {
			for (var e : graph.edges()) {
				edgeAttributes.get(e).setLength(factor * weights.applyAsDouble(e));
			}
		} else { // BoundingCircle
			for (var e : graph.edges()) {
				var ea = edgeAttributes.get(e);
				double L = factor * weights.applyAsDouble(e);
				// include node radii so centers don't collide
				L += nodeAttributes.get(e.getSource()).getRadius()
					 + nodeAttributes.get(e.getTarget()).getRadius();
				ea.setLength(L);
			}
		}
	}

	/* ====================== Main multilevel pipeline ====================== */

	private static void callDivideAndConquerStep(FastMultiLayerMethodOptions options,
												 Graph graph,
												 NodeArray<NodeAttributes> nodeAttributes,
												 EdgeArray<EdgeAttributes> edgeAttributes) {
		int maxLevel = 30;

		// Simple fast paths for chains and cycles (optional)
		...
		var topLevel = MultiLevel.createMultiLevelRepresentations(options, graph, nodeAttributes, edgeAttributes,
				multiLevelGraph, multiLevelNodeAttributes, multiLevelEdgeAttributes);
	...
	private static boolean placeChain(FastMultiLayerMethodOptions options, Graph graph,
									  NodeArray<NodeAttributes> nodeAttributes,
									  EdgeArray<EdgeAttributes> edgeAttributes) {
	...
	private static boolean placeCycle(FastMultiLayerMethodOptions options, Graph graph,
									  NodeArray<NodeAttributes> nodeAttributes,
									  EdgeArray<EdgeAttributes> edgeAttributes) {
		...
		do {
			nodeAttributes.get(v).setPosition(1.1 * (x + 0.1 * options.getUnitEdgeLength() * random.nextDouble()),
					0.6 * (y + 0.1 * options.getUnitEdgeLength() * random.nextDouble()));
			var outEdge = (Edge) null;
			for (Edge e : v.adjacentEdges()) {
				if (e != inEdge) {
					outEdge = e;
	...
	private static void callForceCalculationStep(FastMultiLayerMethodOptions options,
												 LayoutBox layoutBox,
												 Graph graph,
												 NodeArray<NodeAttributes> nodeAttributes,
												 EdgeArray<EdgeAttributes> edgeAttributes,
		...
		if (graph.getNumberOfNodes() > 1) {
			...
			NodeArray<DPoint> lastNodeMovement = graph.newNodeArray(); // stores the last movement to avoid oscillations

			var averageIdealEdgeLength = computeAverageIdealEdgeLength(graph, edgeAttributes);// scaling for forces
			...
			while (((options.getStopCriterion() == FastMultiLayerMethodOptions.StopCriterion.FixedIterations) && (iter <= maxMultiIterations))
				...
				calculateForces(options, graph, layoutBox, averageIdealEdgeLength, coolFactor,
						nodeAttributes, edgeAttributes, forces, forces_attr, forces_rep, lastNodeMovement, iter, options.getFineTuningIterations());
			...
			if (level == 0) {
				...
				callPostprocessingStep(options, graph, layoutBox, averageIdealEdgeLength,
						nodeAttributes, edgeAttributes, forces, forces_attr, forces_rep, lastNodeMovement);
			}
		}
	}
		...
		for (var v : graph.nodes()) {
			...
			if (adjacentNodes.size() == 3) {
				...
				if (direction1Finish.get() == direction2Finish.get() && direction1Steps.get().equals(direction2Steps.get()) && direction1Finish.get() != direction3Finish.get()) {
					...
					path2 = direction2Path; // fixed (was direction1Path)
	...
	private static void callPostprocessingStep(FastMultiLayerMethodOptions options, Graph graph, LayoutBox layoutBox, double averageIdealEdgeLength,
											   NodeArray<NodeAttributes> nodeAttributes, EdgeArray<EdgeAttributes> edgeAttributes,
											   NodeArray<DPoint> F, NodeArray<DPoint> F_attr, NodeArray<DPoint> F_rep, NodeArray<DPoint> lastNodeMovement) {
		{
			var coolFactor = new Single<>(0f);
			for (int i = 1; i <= 10; i++)
	...
	private static void adaptDrawingToIdealAverageEdgeLength(FastMultiLayerMethodOptions options, Graph graph,
															 NodeArray<NodeAttributes> nodeAttributes,
															 EdgeArray<EdgeAttributes> edgeAttributes) {
		...
		for (var e : graph.edges()) {
			...
			sumRealEdgeLength += nodeAttributes.get(e.getTarget()).getPosition()
					.distance(nodeAttributes.get(e.getSource()).getPosition());
		}
		...
		for (var v : graph.nodes()) {
			var va = nodeAttributes.get(v);
			va.setPosition(options.getResizingScalar() * scalingFactor * va.getX(),
					options.getResizingScalar() * scalingFactor * va.getY());
		}
	}
