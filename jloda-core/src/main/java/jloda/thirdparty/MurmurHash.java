/*
 * MurmurHash.java Copyright (C) 2024 Daniel H. Huson
 *
...
/*
 *  MurmurHash.java Copyright (C) third party
 */
...
public final class MurmurHash {
	 ...
	 */
	public static int hash32(final byte[] data, int offset, int length, int seed) {
		...
		for (int i = 0; i < length4; i++) {
			final int i4 = i * 4;
			int k = (data[offset + i4] & 0xff) + ((data[offset + i4 + 1] & 0xff) << 8)
	 ...
	 * @return 32 bit hash of the given array
	 */
	public static int hash32(final byte[] data, int offset, int length) {
		return hash32(data, offset, length, 0x9747b28c);
	}
