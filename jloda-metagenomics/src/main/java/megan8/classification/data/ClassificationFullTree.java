/*
 * ClassificationFullTree.java Copyright (C) 2026 Daniel H. Huson
 *
 ...
 *
 */
package megan8.classification.data;
...
import megan8.algorithms.LCAAddressing;
import megan8.classification.Classification;
import megan8.classification.ClassificationManager;
import megan8.classification.IdMapper;
import megan8.classification.TaxonomyData;
...
public class ClassificationFullTree extends PhyloTree {
	 ...
	 *
	 * @param label short label used only for the progress log message
	 */
	public void loadFromReader(String label, BufferedReader r) throws IOException {
		clear();

		System.err.print("Loading " + label + ": ");
	...
	/**
	 * completes a tree/DAG that has been built directly from a classification database: adds MEGAN's ancillary
	 * nodes, applies the taxonomy-specific fixes and computes the LCA addresses. The caller must already have
	 * created the nodes and edges and set the root.
	 */
	public void finishLoadingFromDatabase() {
		finishLoadingTree();
	}

	/**
	 * adds the ancillary nodes (no-hits, unassigned, low-complexity, contaminants), applies the taxonomy-specific
	 * rank fixes and computes the LCA addresses. Shared by the Newick and the database loading paths.
	 */
	private void finishLoadingTree() {
