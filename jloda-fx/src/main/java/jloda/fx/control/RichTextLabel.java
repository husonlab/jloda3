/*
 * RichTextLabel.java Copyright (C) 2024 Daniel H. Huson
 *
 ...
 * Daniel Huson, 6.2020
 */
public class RichTextLabel extends Group {
	...
	public TextAlignment getTextAlignment() {
		return textFlow.getTextAlignment();
	}
	...
	public double getHeight() {
		return textFlow.getLayoutBounds().getHeight();
	}

	public ReadOnlyDoubleProperty widthProperty() {
		return textFlow.widthProperty();
	}

	public ReadOnlyDoubleProperty heightProperty() {
		return textFlow.heightProperty();
	}
	...
	public void update() {
		...
		if (getGraphic() != null && (getContentDisplay() != ContentDisplay.TOP || getContentDisplay() != ContentDisplay.LEFT))
			textFlow.getChildren().add(getGraphic());
		...
		if (!workingText.isEmpty() && getContentDisplay() != ContentDisplay.GRAPHIC_ONLY) {
			...
			for (var i = 1; i < events.size(); i++) {
				...
				if (event.pos() > segmentStart) {
					...
					textFlow.getChildren().add(textItem);
				}
				...
				if (event.change() == Event.Change.background) {
					...
					if (argument != null) {
						try {
							...
							textFlow.setBackground(new Background(new BackgroundFill(color, null, null)));
						} catch (IllegalArgumentException ignored) {
						}
					}
			...
			if (atEndList != null)
				textFlow.getChildren().addAll(CollectionUtils.reverse(atEndList));
		}
		...
		if (textFlow.getChildren().isEmpty())
			textFlow.getChildren().add(new Text(" ")); // don't want this to be empty
		if (false)
