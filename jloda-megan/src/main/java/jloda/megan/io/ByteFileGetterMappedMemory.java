/*
 * ByteFileGetterMappedMemory.java Copyright (C) 2026 Daniel H. Huson
 *
		...
		if (index < limit()) {
			...
			try {
				...
				return buf.getInt(indexBuffer);
					...
					Exception ex) { // exception is thrown when int goes over buffer boundary. In this case, we need to grab each byte separately
		...
		if (index < limit()) {
			...
			try {
				...
				return buf.getLong(indexBuffer);
					...
					Exception ex) { // exception is thrown when long goes over buffer boundary. In this case, we need to grab each byte separately
