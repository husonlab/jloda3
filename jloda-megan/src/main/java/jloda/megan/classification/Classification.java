/*
 * Classification.java Copyright (C) 2026 Daniel H. Huson
 *
...
public class Classification {
	...
	// load(treeFile, mapFile, progress) removed: legacy file-based loading (used Name2IdMap/ClassificationFullTree
	// loadFromFile + Document version info + Swing error dialogs). Use loadFromDatabase(...) instead.

	/**
	 * load the data for this classification from a classifications database
	 */
	public void loadFromDatabase(IClassificationsDatabase db, ProgressListener progress) {
	...
	public static String createShortTag(String cName) {
		if (cName.equalsIgnoreCase(Taxonomy))
			...
			return "cog|";
		else if (cName.equalsIgnoreCase("go"))
			return "go|";
		else
