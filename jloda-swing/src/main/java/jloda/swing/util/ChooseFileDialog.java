/*
 * ChooseFileDialog.java Copyright (C) 2024 Daniel H. Huson
 *
	...
	public static File chooseFileToOpen(Component parent, File lastOpenFile, FileFilter fileFilter, FilenameFilter fileNameFilter, ActionEvent event, String message) {
		...
		if (ProgramProperties.isMacOS() && (event == null || (event.getModifiers() & ActionEvent.SHIFT_MASK) == 0)) {
			...
			if (parent instanceof JFrame)
				dialog = new FileDialog((JFrame) parent, message, FileDialog.LOAD);
			else if (parent instanceof Dialog)
				dialog = new FileDialog((Dialog) parent, message, FileDialog.LOAD);
			else
				dialog = new FileDialog((JFrame) null, message, FileDialog.LOAD);
	...
	public static File chooseFileToSave(Component parent, File lastOpenFile, FileFilter fileFilter, FilenameFilter fileNameFilter, ActionEvent event, String message, String defaultSuffix) {
		...
		while (!okToWrite) {
			if (ProgramProperties.isMacOS() && (event == null || (event.getModifiers() & ActionEvent.SHIFT_MASK) == 0)) {
				...
				FileDialog dialog;
				if (parent instanceof JFrame)
					dialog = new FileDialog((JFrame) parent, message, FileDialog.SAVE);
				else if (parent instanceof Dialog)
					dialog = new FileDialog((Dialog) parent, message, FileDialog.SAVE);
				else
					dialog = new FileDialog((JFrame) null, message, FileDialog.SAVE);
