/*
 * EdgeView.java Copyright (C) 2024 Daniel H. Huson
 *
...
final public class EdgeView extends ViewBase { //, IEdgeView {
	...
	private List<Point2D> internalPoints = null;
	...
	public void draw(Graphics2D gc, Point vp, Point wp, Transform trans, boolean hilited) {
		...
		if (hilited) {// draw edge highlighting first
			...
			if (shape == STRAIGHT_EDGE || getInternalPoints() == null || getInternalPoints().isEmpty()) {
				...
				final double dist = trans.w2d(ROUNDED_EDGE_INCREMENT, 0).getX() - trans.w2d(0, 0).getX();
				final Point center = new Point(dist < wp.x - vp.x ? (int) Math.round(vp.x + dist) : wp.x, wp.y);
	...
	public void drawLabel(Graphics2D gc, Transform trans, boolean hilited) {
		if (this.isLabelVisible() && labelColor != null && label != null) {
			...
			if (labelAngle == 0) {
				if (apt != null)
					gc.drawString(label, Math.round(apt.getX()), Math.round(apt.getY()));
			} else {
	...
	public void setLabelReferencePosition(Point vp, Point wp, Transform trans) {
		if (shape != STRAIGHT_EDGE && getInternalPoints() != null && !getInternalPoints().isEmpty()) {
			...
			int x = Math.round(0.5f * (vp.x + wp.x));
			int y = Math.round(0.5f * (vp.y + wp.y));
	...
	public Shape getLabelShape(Transform trans) {
		if (labelSize != null) {
			...
			if (apt != null) {
				if (labelAngle == 0) {
					return new Rectangle((int) Math.round(apt.getX()), (int) Math.round(apt.getY() - labelSize.height + 1), labelSize.width, labelSize.height);
				} else {
					...
					return new Polygon(new int[]{(int) Math.round(pts[0]), (int) Math.round(pts[2]), (int) Math.round(pts[4]), (int) Math.round(pts[6])},
							new int[]{(int) Math.round(pts[1]), (int) Math.round(pts[3]), (int) Math.round(pts[5]), (int) Math.round(pts[7])}, 4);
				}
			}
		}
		return null;
	...
	public String toString(EdgeView previousEV, boolean withInternalPoints, boolean withLabel) {
		...
		if (withInternalPoints && internalPoints != null && !internalPoints.isEmpty()) {
			buf.append(" ip= <");
		...
		if (withLabel && label != null && !label.isEmpty())
	 ...
	 */
	public void read(NexusStreamParser np, List<String> tokens, EdgeView prevEV) throws IOException {
		if (prevEV == null)
		...
		if (internalPointsStr != null && !internalPointsStr.isEmpty()) {
			try {
