/*
 * TanglegramDisplacementOptimization.java Copyright (C) 2026 Daniel H. Huson
 *
...
public class TanglegramDisplacementOptimization {
	 ...
	 * @param canceled              returns true if computation is to be canceled
	 * @param <Node>                tree nodes
	 * @return new node-to-list-of children mapping for phylogeny 2
	 */
	public static <Node> ResultPair<Map<Node, List<Node>>> apply(Node root1, Function<Node, Integer> nodeTaxonFunction1, Function<Node, List<Node>> nodeChildrenFunction1,
																 Node root2, Function<Node, Integer> nodeTaxonFunction2, Function<Node, List<Node>> nodeChildrenFunction2,
																 Random random, BooleanSupplier canceled) {
		return apply(root1, nodeTaxonFunction1, nodeChildrenFunction1, root2, nodeTaxonFunction2, nodeChildrenFunction2, Collections.emptyMap(), true, false, random, canceled);
	}

	/**
	 * compute a displacement optimized tanglegram for tree (or network) 2 while keeping tree (or network) 1 fixed
	 *
